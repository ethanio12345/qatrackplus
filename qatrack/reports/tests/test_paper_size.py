...
class TestPaperSizeInReports(TestCase):
    ...
    def test_saved_report_default_paper_size(self):
        ...
        self.assertEqual(report.paper_size, 'letter', "Default paper size should be 'letter'")
    ...
    def test_saved_report_a4_paper_size(self):
        ...
        self.assertEqual(report.paper_size, 'a4', "Paper size should be set to 'a4'")
    ...
    def test_report_form_default_paper_size(self):
        ...
        form = ReportForm()
        self.assertEqual(form.fields['paper_size'].initial, 'letter', "Form should default to letter size")
    ...
    def test_report_form_validation_with_paper_size(self):
        ...
        self.assertTrue(form.is_valid(), f"Form should be valid with A4 paper size. Errors: {form.errors}")
    ...
    def test_report_form_clean_paper_size_default(self):
        ...
        form = ReportForm(data=form_data, prefix='root')
        if form.is_valid():
            self.assertEqual(form.cleaned_data['paper_size'], 'letter', "Clean method should default to letter size")
    ...
    def test_base_report_context_includes_paper_size_letter(self):
        ...
        self.assertEqual(context['paper_size'], 'letter', "Context should include letter paper size")
    ...
    def test_base_report_context_includes_paper_size_a4(self):
        ...
        self.assertIn('paper_size', context)
        self.assertEqual(context['paper_size'], 'a4', "Context should include A4 paper size")
    ...
    def test_base_report_context_default_paper_size(self):
        ...
        self.assertIn('paper_size', context)
        self.assertEqual(context['paper_size'], 'letter', "Context should default to letter paper size")
    ...
    def test_pdf_generation_with_letter_size(self, mock_chrometopdf):
        ...
        self.assertEqual(call_args[1]['paper_size'], 'letter', "chrometopdf should be called with letter paper size")
    ...
    def test_pdf_generation_with_a4_size(self, mock_chrometopdf):
        ...
        mock_chrometopdf.assert_called_once()
        call_args = mock_chrometopdf.call_args
        self.assertEqual(call_args[1]['paper_size'], 'a4', "chrometopdf should be called with A4 paper size")
...
class TestChromeToPdfPaperSize(TestCase):
    ...
    def test_chrometopdf_letter_paper_size(self, mock_open, mock_subprocess):
        ...
        with mock.patch('qatrack.qatrack_core.utils.settings') as mock_settings:
            ...
            self.assertIn('--print-to-pdf-paper-format=Letter', command, "Command should include Letter paper format")
    ...
    def test_chrometopdf_a4_paper_size(self, mock_open, mock_subprocess):
        ...
        with mock.patch('qatrack.qatrack_core.utils.settings') as mock_settings:
            ...
            self.assertIn('--print-to-pdf-paper-format=A4', command, "Command should include A4 paper format")
    ...
    def test_chrometopdf_default_paper_size(self, mock_open, mock_subprocess):
        ...
        with mock.patch('qatrack.qatrack_core.utils.settings') as mock_settings:
            ...
            self.assertIn(
                '--print-to-pdf-paper-format=Letter', command, "Command should default to Letter paper format"
            )
...
class TestActualPDFDimensions(TestCase):
    ...
    def test_pdf_export_dimensions_are_different(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pdf') as letter_file, \
            ...
            self.assertNotEqual(letter_width, a4_width, "Letter and A4 PDFs should have different widths")
            self.assertNotEqual(letter_height, a4_height, "Letter and A4 PDFs should have different heights")

            # More specific checks
            self.assertAlmostEqual(
                letter_w_in, 8.5, places=0, msg=f"Letter width should be ~8.5\", got {letter_w_in:.1f}\""
            )
            self.assertAlmostEqual(
                letter_h_in, 11.0, places=0, msg=f"Letter height should be ~11.0\", got {letter_h_in:.1f}\""
            )

            # A4 is 210mm × 297mm = 8.27\" × 11.69\"
            self.assertAlmostEqual(a4_w_in, 8.3, places=0, msg=f"A4 width should be ~8.3\", got {a4_w_in:.1f}\"")
            self.assertAlmostEqual(a4_h_in, 11.7, places=0, msg=f"A4 height should be ~11.7\", got {a4_h_in:.1f}\"")
        ...
        with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_pdf:
            try:
                ...
                with open(temp_pdf.name, 'rb') as pdf_file:
                    ...
                    self.assertAlmostEqual(
                        width_inches, 8.5, places=1, msg=f"PDF width should be ~8.5 inches, got {width_inches}"
                    )
                    self.assertAlmostEqual(
                        height_inches, 11.0, places=1, msg=f"PDF height should be ~11 inches, got {height_inches}"
                    )
    ...
    def test_pdf_dimension_comparison(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as letter_file, \
            ...
            try:
                ...
                self.assertGreater(letter_width, a4_width, "Letter should be wider than A4")
                self.assertLess(letter_height, a4_height, "Letter should be shorter than A4")
...
class TestPaperSizeIntegration(TestCase):
    ...
    def test_saved_report_pdf_generation_letter(self):
        ...
        base_opts = report.base_opts
        self.assertEqual(base_opts['paper_size'], 'letter', "base_opts should include letter paper size")
    ...
    def test_saved_report_pdf_generation_a4(self):
        ...
        # Test the base_opts include paper_size
        base_opts = report.base_opts
        self.assertEqual(base_opts['paper_size'], 'a4', "base_opts should include A4 paper size")
    ...
    def test_paper_size_choices_valid(self):
        ...
        for choice in model_choices:
            self.assertIn(choice, valid_choices, f"Paper size choice '{choice}' should be valid")

        self.assertEqual(len(model_choices), 2, "Should have exactly 2 paper size choices")

    def test_paper_size_display_names(self):
        """Test that paper size choices have proper display names."""
        expected_displays = {'letter': 'Letter (8.5" × 11")', 'a4': 'A4 (210mm × 297mm)'}

        for choice_value, choice_display in SavedReport.PAPER_SIZES:
            self.assertEqual(
                str(choice_display), expected_displays[choice_value],
    ...
    def test_saved_report_with_different_formats(self):
        ...
        for report_format in ['pdf', 'xlsx', 'csv']:
            with self.subTest(format=report_format):
                ...
                self.assertEqual(report.paper_size, 'a4', f"Paper size should be A4 for {report_format} format")

                base_opts = report.base_opts
                self.assertEqual(base_opts['paper_size'], 'a4', f"base_opts should include A4 for {report_format}")
