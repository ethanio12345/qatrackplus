...
```
Id               uniqueidentifier PK (also FK -> TIE)
Tolerance_Warn    float  -- UNVERIFIED (schema-dump-request.md Step 5.5 not yet run); may actually be WarningTolerance or WarnOn. Wedge is out of scope, so this does not block the change.
Tolerance_Fail    float  -- UNVERIFIED; may actually be ErrorTolerance or FailOn
...
- `5.Tmt.Linac.D%` matches BOTH `5.Tmt.Linac.D` and `5.Tmt.Linac.D2` because `%` swallows the `2 - Daily QA (Physics)` suffix. (`.` is a LITERAL in T-SQL `LIKE`; only `%` and `_` are wildcards.) This means a single `.D%` filter cannot distinguish Constancy from Physics — see explore-brief.md decision D1.
